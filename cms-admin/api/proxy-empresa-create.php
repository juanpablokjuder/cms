...
$data  = [];
foreach (['nombre','telefono','mail','direccion'] as $f) {
    if (array_key_exists($f, $input)) $data[$f] = $input[$f];
}
if (empty($data['nombre'])) { jsonResponse(['success'=>false,'message'=>'El nombre es requerido.'],422); }
$result = apiRequest('POST', '/empresa', $data);
