...
$data = [];
foreach (['nombre', 'descripcion', 'direccion', 'telefono'] as $field) {
    if (array_key_exists($field, $input))
        $data[$field] = $input[$field];
}
if (array_key_exists('horarios', $input) && is_array($input['horarios'])) {
    $data['horarios'] = $input['horarios'];
}
...
$result = apiRequest('PATCH', '/locales/' . urlencode($uuid), $data);
