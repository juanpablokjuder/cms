...
$atributos = isset($input['atributos']) && is_array($input['atributos']) ? $input['atributos'] : null;

if (!$nombre) {
    jsonResponse(['success' => false, 'message' => 'El campo nombre es requerido.'], 400);
}
if (!$atributos) {
    jsonResponse(['success' => false, 'message' => 'El campo atributos es requerido.'], 400);
}

$result = apiRequest('POST', '/productos/atributos', ['nombre' => $nombre, 'atributos' => $atributos]);
