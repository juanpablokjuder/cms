...
function api_get(string $endpoint, array $params = []): mixed
{
    ...
    if (!isset($json['success']) || $json['success'] !== true) return null;
...
function api_productos(array $opts = []): array
{
    ...
    if (!empty($opts['search']))            $params['search'] = $opts['search'];
    if (!empty($opts['marcas']))            $params['marcas'] = is_array($opts['marcas']) ? implode(',', $opts['marcas']) : $opts['marcas'];
