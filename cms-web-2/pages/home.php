...
$cmsSeo = api_seo('pagina', 'home');
$currentRoute = 'home';

// SEO base con fallback a imagen del primer banner o nosotros
$seoImage = '';
...
<!-- ════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════ -->
<section class="vm-hero" aria-label="Bienvenida" tabindex="0">

    <!-- Track: todas las diapositivas apiladas -->
    <div class="vm-hero-track">
        <?php
        if (!empty($banners)) {
            ...
            foreach ($banners as $i => $b):
                ...
                ?>
                <div class="vm-hero-slide <?= $i === 0 ? 'is-active' : '' ?>" data-index="<?= $i ?>">
                    <div class="vm-hero-bg" aria-hidden="true">
                        <?php if ($heroImg): ?>
                            <div class="vm-hero-bg-img" style="background-image:url('<?= e($heroImg) ?>')"></div>
                        <?php endif; ?>
                        <div class="vm-hero-overlay"></div>
                    </div>
                    <div class="vm-hero-content">
                        <div class="vm-container">
                            <div class="vm-hero-content-inner">
                                ...
                                <div class="vm-hero-actions">
                                    <?php if (!empty($b['btn_texto']) && !empty($b['btn_link'])): ?>
                                        <a href="<?= e($b['btn_link']) ?>"
                                            class="vm-btn vm-btn-cta vm-btn-lg"><?= e($b['btn_texto']) ?></a>
                                    <?php else: ?>
            ...
            <!-- Fallback cuando no hay banners en el CMS -->
            <div class="vm-hero-slide is-active">
                <div class="vm-hero-bg" aria-hidden="true">
                    <div class="vm-hero-overlay"></div>
                </div>
                <div class="vm-hero-content">
                    <div class="vm-container">
                        <div class="vm-hero-content-inner">
                            ...
                            <p>Descubrí los smartphones más avanzados del mercado con garantía oficial, financiación y
                                servicio post-venta directo.</p>
                            <div class="vm-hero-actions">
    ...
    <?php if (count($banners) > 1): ?>
        ...
        <button type="button" class="vm-hero-arrow vm-hero-arrow--prev" aria-label="Slide anterior">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <polyline points="15 18 9 12 15 6" />
            </svg>
        </button>
        <button type="button" class="vm-hero-arrow vm-hero-arrow--next" aria-label="Slide siguiente">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <!-- Navegación inferior: barra de progreso + dots -->
        <div class="vm-hero-nav" role="group" aria-label="Controles del slider">
            <div class="vm-hero-progress" aria-hidden="true">
                <div class="vm-hero-progress-bar" id="vm-hero-progress-bar"></div>
            </div>
            <div class="vm-hero-dots" role="tablist" aria-label="Banners">
                <?php foreach ($banners as $i => $_): ?>
                    <button type="button" class="vm-hero-dot <?= $i === 0 ? 'is-active' : '' ?>" role="tab"
                        aria-selected="<?= $i === 0 ? 'true' : 'false' ?>" aria-label="Banner <?= $i + 1 ?>"
                        data-go="<?= $i ?>"></button>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <div class="vm-container">
        <div class="vm-section-head">
            <div>
                ...
                <h2 id="servicios-h">
                    <?= e($servicios['servicio']['titulo'] ?? 'Logística, garantía y soporte de fábrica') ?>
                </h2>
                    ...
                    <article class="vm-card p-6 vm-reveal">
                        ...
                        <ul class="space-y-3 mt-4">
                            <?php foreach (array_slice($cat['items'] ?? [], 0, 4) as $item): ?>
                                <li class="flex gap-3">
                                    <span class="w-1.5 h-1.5 rounded-full bg-cta mt-2 flex-shrink-0"
                                        style="background:var(--vm-cta)"></span>
                                    <div>
            ...
            <div class="grid gap-6 md:grid-cols-3">
                <article class="vm-card p-6">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-4"
                        style="background:rgba(0,33,71,0.06);color:var(--vm-primary)">
                    ...
                    <p class="text-slate-600 text-sm">Entrega en 24-72h en CABA y GBA. Envíos a todo el país con
                        seguimiento.</p>
                </article>
                <article class="vm-card p-6">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-4"
                    ...
                    <p class="text-slate-600 text-sm">12 meses de garantía oficial del fabricante. Servicio técnico
                        autorizado.</p>
                </article>
                <article class="vm-card p-6">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-4"
                    ...
                    <p class="text-slate-600 text-sm">Financiación con todas las tarjetas. Promociones bancarias vigentes.
                    </p>
                </article>
            </div>
        <?php endif; ?>
                ...
                <?php if ($img = first_image($nosotros['imagenes'] ?? [])): ?>
                    <div class="vm-reveal">
                        <div class="overflow-hidden rounded-3xl shadow-xl aspect-[4/5]">
                            <img src="<?= e($img['url']) ?>" alt="<?= e($img['alt'] ?? $nosotros['titulo']) ?>" loading="lazy"
    ...
    <div class="vm-container">
        <div class="grid gap-10 lg:grid-cols-2 items-start">
            <div>
                ...
                <p class="mt-4 text-lg opacity-80 max-w-xl">Nuestros asesores te ayudan a encontrar el smartphone ideal
                    según tu uso, presupuesto y financiación.</p>
                        ...
                        <li class="flex gap-3 items-center">
                            <span class="w-10 h-10 rounded-full flex items-center justify-center"
                                ...
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                                    <path
                                        d="M17.6 6.32A8 8 0 0 0 4 12a8 8 0 0 0 1.18 4.18L4 22l5.96-1.16A8 8 0 0 0 20 12a8 8 0 0 0-2.4-5.68z" />
                                </svg>
                            </span>
                            <a href="https://wa.me/<?= e(SITE_WHATSAPP) ?>" target="_blank" rel="noopener"
                                class="hover:text-cta transition-colors">
                                WhatsApp
                            </a>
                        </li>
                    <?php endif; ?>
            ...
            <form id="vm-contact-form" class="vm-form bg-white/5 p-8 rounded-2xl border border-white/10" novalidate>
                <div class="vm-form-row cols-2">
                    <div>
                        <label for="cf-name" class="vm-form-label" style="color:rgba(255,255,255,.85)">Nombre <span
                                class="required">*</span></label>
                        <input type="text" id="cf-name" name="name" class="vm-input" required minlength="2"
                            maxlength="80" autocomplete="name">
                        <p class="vm-error-msg" data-error="name"></p>
                    </div>
                    <div>
                        <label for="cf-email" class="vm-form-label" style="color:rgba(255,255,255,.85)">Email <span
                                class="required">*</span></label>
                        <input type="email" id="cf-email" name="email" class="vm-input" required maxlength="120"
                            autocomplete="email">
                        <p class="vm-error-msg" data-error="email"></p>
                    </div>
                </div>
                <div>
                    <label for="cf-message" class="vm-form-label" style="color:rgba(255,255,255,.85)">Mensaje <span
                            class="required">*</span></label>
                    <textarea id="cf-message" name="message" class="vm-input" required minlength="10" maxlength="2000"
                        rows="5"></textarea>
